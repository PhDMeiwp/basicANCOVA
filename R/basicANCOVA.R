#' One-way Analysis of Covariance Judgement and Plot
#'
...
#' @param ... additional parameters to \code{\link[graphics]{plot}}, such as main, sub.
#'
#' @import graphics
#' @import stats
#' @import car
#' @export
#' @details The "Result 2" (Call: aov(formula = y ~ x + group, data = data, contrasts = list(group = contr.sum))) of \code{\link[basicANCOVA]{ANCOVAplot}} function outputs the values of common slope, common intercept and specific intercepts of only n-1 groups (not all groups). \cr Whereas the "Result 3" (Call: aov(formula = y ~ x + group, data = data, contrasts = list(group = contr.treatment))) outputs the values of common slope and specific intercepts of all groups.
#'
#' @examples
#'
#' library(basicANCOVA)
